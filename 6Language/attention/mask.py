...
def get_color_for_attention_score(attention_score):
    ...
    """
    rgb = round(float(attention_score) * 255)
...
def visualize_attentions(tokens, attentions):
    ...
    """
    num_layers = len(attentions)  # number of attention layers

    for layer_index in range(num_layers):
        # attentions[layer_idx] has shape: (batch_size, num_heads, seq_len, seq_len)
        heads = attentions[layer_index].shape[1]

        for head_index in range(heads):
            # Convert from 0-indexed to 1-indexed for diagram labels
            layer_number = layer_index + 1
            head_number = head_index + 1

            # Extract attention matrix for this layer/head
            attention_matrix = attentions[layer_index][0][head_index]

            # Call provided diagram function
